...
const wordList = [
    ...
    // Add the remaining 119 words with their descriptions here
];
...
function generateCrossword() {
    const selectedWords = shuffle(wordList).slice(0, 24);
...
// Function to display word description
function displayDescription(word) {
    const description = wordList.find(wordObj => wordObj.word === word).description;
...
// Event listener for crossword word selection
document.getElementById("puzzle").addEventListener("click", (event) => {
    const selectedWord = event.target.textContent;
    displayDescription(selectedWord);
});

// Generate the crossword puzzle and start the timer
generateCrossword();
startTimer()
